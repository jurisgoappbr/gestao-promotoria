...
// ─── API SUPABASE (REST direto, sem SDK) ──────────────────────────────────────
...
function DynSelect({ value, onChange, options, onAdd, placeholder, style:st }) {
  ...
  const confirm = () => { if (nv.trim()) { onAdd(nv.trim()); onChange(nv.trim()); setAdding(false); setNv(""); } };
  if (adding) return (
    <div style={{ display:"flex", gap:4, ...st }}>
      <input style={{ ...S.input, flex:1 }} value={nv} onChange={(e)=>setNv(e.target.value)} placeholder="Novo valor..." autoFocus
        onKeyDown={(e)=>{ if(e.key==="Enter") confirm(); if(e.key==="Escape"){ setAdding(false); setNv(""); } }} />
      <button style={S.btn("success")} onClick={confirm}><Check size={13}/></button>
      <button style={S.btn("ghost")} onClick={()=>{ setAdding(false); setNv(""); }}><X size={13}/></button>
    </div>
  );
  return (
    <select style={{ ...S.select, width:"100%", ...st }} value={value}
  ...
  return (
    <div style={{ ...S.page, alignItems:"center", justifyContent:"center", background:"#0f172a" }}>
      ...
      <div style={{ background:"#fff", borderRadius:16, padding:36, width:400, maxWidth:"90vw", textAlign:"center" }}>
        ...
        <button style={{ ...S.btn("primary"), width:"100%", justifyContent:"center", padding:"9px", marginBottom:10, opacity:url&&key?1:0.5 }}
          disabled={!url||!key} onClick={()=>onConnect(url,key)}>Conectar ao Supabase</button>
...
function AuthScreen({ api, onAuth }) {
  ...
  const [tipo, setTipo]       = useState("graduacao");
  ...
  const submit = async () => {
    ...
    try {
      if (mode === "login") {
        ...
        const profiles = await api.get("profiles", r.access_token, `id=eq.${userId}`);
        const prof = Array.isArray(profiles) ? profiles[0] : null;
        if (!prof) { setErr("Perfil não encontrado."); setLoading(false); return; }
        onAuth(r.access_token, prof);
      } else {
        const r = await api.signup(email, pw);
        if (r.error || r.message) { setErr(r.error_description||r.message||"Erro no registro"); setLoading(false); return; }
        // Tenta usar o token retornado no signup; se não tiver, faz login
        let tok = r.access_token;
        let uid = r.user?.id;
        if (!tok) {
          const login = await api.login(email, pw);
          tok = login.access_token;
          uid = login.user?.id;
        }
        if (!uid || !tok) { setErr("Erro ao criar conta. Tente fazer login."); setLoading(false); return; }
        await api.post("profiles", { id:uid, nome, email, papel:"estagiaria", tipo_estagiaria:tipo, carga_horaria_diaria:parseFloat(carga) }, tok);
        const profiles = await api.get("profiles", tok, `id=eq.${uid}`);
        const prof = Array.isArray(profiles) ? profiles[0] : null;
        if (!prof) { setErr("Perfil criado. Faça login."); setLoading(false); setMode("login"); return; }
        onAuth(tok, prof);
      }
  ...
  return (
    <div style={{ ...S.page, alignItems:"center", justifyContent:"center", background:"#0f172a" }}>
      ...
      <div style={{ background:"#fff", borderRadius:16, padding:36, width:400, maxWidth:"90vw" }}>
        ...
        {mode==="register" && <Field label="Nome completo"><input style={S.input} value={nome} onChange={(e)=>setNome(e.target.value)}/></Field>}
        <Field label="E-mail"><input style={S.input} type="email" value={email} onChange={(e)=>setEmail(e.target.value)}/></Field>
        <Field label="Senha"><input style={S.input} type="password" value={pw} onChange={(e)=>setPw(e.target.value)}
          onKeyDown={(e)=>{ if(e.key==="Enter") submit(); }}/></Field>
        {mode==="register" && <>
          <Field label="Tipo">
            <select style={{ ...S.select, width:"100%" }} value={tipo} onChange={(e)=>setTipo(e.target.value)}>
              <option value="graduacao">Graduação (4h)</option>
              <option value="pos">Pós-Graduação (6h)</option>
              <option value="voluntaria">Voluntária</option>
            </select>
          </Field>
          <Field label="Carga horária/dia">
            <input style={S.input} type="number" step="0.5" min="1" max="8" value={carga} onChange={(e)=>setCarga(e.target.value)}/>
          </Field>
        </>}
        {err && <div style={{ color:"#ef4444", fontSize:12, marginBottom:10 }}>{err}</div>}
        <button style={{ ...S.btn("primary"), width:"100%", justifyContent:"center", padding:"9px", marginBottom:10 }}
          onClick={submit} disabled={loading}>{loading?"Aguarde...":mode==="login"?"Entrar":"Registrar"}</button>
        <div style={{ textAlign:"center", fontSize:12.5, color:"#64748b" }}>
          {mode==="login"
            ? <span>Nova estagiária? <a href="#" style={{ color:"#2563eb" }} onClick={(e)=>{ e.preventDefault(); setMode("register"); setErr(""); }}>Registre-se</a></span>
            : <span>Já tem conta? <a href="#" style={{ color:"#2563eb" }} onClick={(e)=>{ e.preventDefault(); setMode("login"); setErr(""); }}>Entrar</a></span>}
        </div>
      </div>
    </div>
  );
...
function Sidebar({ papel, active, setActive, nome, pendCount, onLogout }) {
  ...
  return (
    <div style={S.sidebar}>
      ...
      {tabs.map((t)=>(
        <div key={t.key} style={S.navItem(active===t.key)} onClick={()=>setActive(t.key)}>
...
function BacklogCard({ backlog, setBacklog, selectedDate, api, token, demo }) {
  ...
  const emptyLocal = { pecas_corrigir:"", pecas_minhas:"", pecas_estagiarias:"" };
  ...
  const [localInicio, setLocalInicio] = useState(blInicio ? { pecas_corrigir:String(blInicio.pecas_corrigir), pecas_minhas:String(blInicio.pecas_minhas), pecas_estagiarias:String(blInicio.pecas_estagiarias) } : emptyLocal);
  const [localFim,    setLocalFim]    = useState(blFim    ? { pecas_corrigir:String(blFim.pecas_corrigir),    pecas_minhas:String(blFim.pecas_minhas),    pecas_estagiarias:String(blFim.pecas_estagiarias)    } : emptyLocal);
  const [savingI, setSavingI] = useState(false);
  const [savingF, setSavingF] = useState(false);
  const [savedI,  setSavedI]  = useState(false);
  const [savedF,  setSavedF]  = useState(false);

  // Sincroniza campos locais quando a data muda ou dados chegam do banco
  ...
  useEffect(() => {
    ...
    setLocalInicio(i ? { pecas_corrigir:String(i.pecas_corrigir), pecas_minhas:String(i.pecas_minhas), pecas_estagiarias:String(i.pecas_estagiarias) } : emptyLocal);
    setLocalFim(f    ? { pecas_corrigir:String(f.pecas_corrigir), pecas_minhas:String(f.pecas_minhas), pecas_estagiarias:String(f.pecas_estagiarias) } : emptyLocal);
  ...
  const saveMomento = async (momento) => {
    ...
    const setSaving= momento==="inicio" ? setSavingI  : setSavingF;
    const setSaved = momento==="inicio" ? setSavedI   : setSavedF;
    const payload  = {
      data: selectedDate, momento,
    ...
    const updateLocal = (row) => {
      if (ex) setBacklog((prev)=>prev.map((b)=>(b.data===selectedDate&&b.momento===momento)?{ ...b, ...payload, id:b.id }:b));
      else    setBacklog((prev)=>[...prev, row]);
    };
    if (!demo) {
      try {
        if (ex) {
          ...
          updateLocal(row ?? { ...payload, id:Date.now() });
        }
      } catch (e) {
        console.error("Erro ao salvar backlog:", e);
        updateLocal({ ...payload, id:ex?.id ?? Date.now() });
      }
    } else {
      updateLocal({ ...payload, id:ex?.id ?? Date.now() });
    }
        ...
        {BL_FIELDS.map((f)=>(
          <Field key={f.key} label={f.label} style={{ marginBottom:0, flex:1 }}>
            <input type="number" min="0" style={{ ...S.input, textAlign:"center" }}
              value={local[f.key]} placeholder="0"
              onChange={(e)=>setLocal((prev)=>({ ...prev, [f.key]:e.target.value }))}/>
          </Field>
        ))}
      </div>
      <button style={{ ...S.btn(saved?"success":"primary"), fontSize:11, padding:"5px 10px", width:"100%", justifyContent:"center" }}
        onClick={()=>saveMomento(momento)} disabled={saving}>
  ...
  return (
    <div style={{ ...S.card, padding:"14px 16px" }}>
      ...
      <div style={{ display:"flex", gap:18, flexWrap:"wrap", alignItems:"flex-start" }}>
        <Section momento="inicio" label="Início do dia" icon={Sunrise} color="#f59e0b" local={localInicio} setLocal={setLocalInicio} saving={savingI} saved={savedI}/>
        <div style={{ width:1, background:"#e2e8f0", alignSelf:"stretch", marginTop:20 }}/>
        <Section momento="fim"    label="Fim do dia"    icon={Sunset}  color="#6366f1" local={localFim}    setLocal={setLocalFim}    saving={savingF} saved={savedF}/>
        {(blInicio||blFim) && (
      ...
      {blInicio && blFim && (() => {
        const tI = (blInicio.pecas_corrigir||0)+(blInicio.pecas_minhas||0)+(blInicio.pecas_estagiarias||0);
        const tF = (blFim.pecas_corrigir||0)+(blFim.pecas_minhas||0)+(blFim.pecas_estagiarias||0);
        const diff = tF-tI;
        if (!tI && !tF) return null;
        return (
          <div style={{ marginTop:12, paddingTop:10, borderTop:"1px solid #f1f5f9", fontSize:12, color:"#475569" }}>
            Pendências totais: <strong>{tI}</strong> → <strong>{tF}</strong>
            {diff!==0 && <span style={{ marginLeft:6, fontWeight:700, color:diff<0?"#10b981":"#ef4444" }}>({diff<0?`${Math.abs(diff)} a menos`:`${diff} a mais`})</span>}
          </div>
        );
      })()}
    </div>
...
function DiaTrabalho({ registros, setRegistros, entregas, setEntregas, backlog, setBacklog, opts, setOpts, estagiarias, selectedDate, setSelectedDate, api, token, demo }) {
  ...
  const closeForm = () => { setShowForm(false); setCorrigindo(null); setEditId(null); setForm(emptyForm); };
  ...
  const saveReg = async () => {
    ...
    if (editId) {
      ...
      setRegistros((prev)=>prev.map((r)=>r.id===editId?{ ...r,...clean }:r));
    } else {
      ...
      if (!demo) {
        try {
          ...
          if (corrigindo) {
            ...
            setEntregas((prev)=>prev.map((e)=>e.id===corrigindo.id?{ ...e, status:"corrigido" }:e));
          }
        } catch(e) {
          setRegistros((prev)=>[...prev, { ...payload, id:Date.now() }]);
        }
      } else {
        setRegistros((prev)=>[...prev, { ...payload, id:Date.now() }]);
        if (corrigindo) setEntregas((prev)=>prev.map((e)=>e.id===corrigindo.id?{ ...e, status:"corrigido" }:e));
      }
    }
    closeForm();
  };
  ...
  const delReg = async (id) => {
    ...
    if (r?.entrega_id) setEntregas((prev)=>prev.map((e)=>e.id===r.entrega_id?{ ...e, status:"pendente" }:e));
  ...
  return (
    <div>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:14 }}>
        ...
        <button style={S.btn("primary")} onClick={()=>{ setForm(emptyForm); setEditId(null); setCorrigindo(null); setShowForm(true); }}><Plus size={14}/> Novo Registro</button>
      </div>

      <BacklogCard backlog={backlog} setBacklog={setBacklog} selectedDate={selectedDate} api={api} token={token} demo={demo}/>
      ...
      {pendentes.length>0 && (
        <div style={{ ...S.card, borderLeft:"4px solid #f59e0b" }}>
          ...
          {pendentes.map((e)=>(
            <div key={e.id} style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"7px 0", borderBottom:"1px solid #f8fafc" }}>
        ...
        {dayRegs.length===0 ? (
          ...
          <div style={{ overflowX:"auto" }}>
            <table style={{ width:"100%", borderCollapse:"collapse", minWidth:800 }}>
              <thead><tr>
                <th style={S.th}>Número</th><th style={S.th}>Tipo</th><th style={S.th}>Vista</th><th style={S.th}>Crime</th>
                <th style={S.th}>Manifestação</th><th style={S.th}>Resp.</th><th style={S.th}>Correção</th><th style={S.th}>Obs.</th><th style={S.th}>Flags</th><th style={S.th}></th>
              </tr></thead>
              <tbody>
                {dayRegs.map((r)=>(
                  <tr key={r.id} style={{ background:r.responsavel!=="Igor"?"#f0f9ff":"transparent" }}>
                    ...
                    <td style={S.td}>
                      {r.acompanhar  && <span style={{ ...S.badge("#1e40af","#dbeafe"), marginRight:3 }}>AC</span>}
                      {r.complicado  && <span style={{ ...S.badge("#991b1b","#fee2e2"), marginRight:3 }}>!</span>}
                      {r.feedback_tipo && <span style={S.badge(r.feedback_dado?"#065f46":"#92400e",r.feedback_dado?"#d1fae5":"#fef3c7")}>{r.feedback_tipo==="elogio"?"👍":"👎"}{r.feedback_dado?" ✓":""}</span>}
                    </td>
                    <td style={S.td}>
                      <div style={{ display:"flex", gap:4 }}>
                        <Edit3 size={13} style={{ cursor:"pointer", color:"#94a3b8" }} onClick={()=>startEdit(r)}/>
                        <Trash2 size={13} style={{ cursor:"pointer", color:"#94a3b8" }} onClick={()=>delReg(r.id)}/>
                      </div>
                    </td>
                  </tr>
                ))}
      ...
      {showForm && (
        <Modal title={corrigindo?"Corrigir Entrega":editId?"Editar Registro":"Novo Registro"} onClose={closeForm}>
          ...
          <div style={{ display:"flex", gap:10 }}>
            <Field label="Tipo procedimento *" style={{ flex:1 }}>
              <DynSelect value={form.tipo_procedimento} onChange={(v)=>upd("tipo_procedimento",v)} options={opts.tipo_procedimento} onAdd={(v)=>{ setOpts((p)=>({ ...p, tipo_procedimento:[...p.tipo_procedimento,v] })); addOpt("tipo_procedimento",v); }}/>
            </Field>
          ...
          <div style={{ display:"flex", gap:10 }}>
            <Field label="Tipo manifestação *" style={{ flex:1 }}>
              <DynSelect value={form.tipo_manifestacao} onChange={(v)=>upd("tipo_manifestacao",v)} options={opts.tipo_manifestacao} onAdd={(v)=>{ setOpts((p)=>({ ...p, tipo_manifestacao:[...p.tipo_manifestacao,v] })); addOpt("tipo_manifestacao",v); }}/>
            </Field>
            <Field label="Responsável" style={{ flex:1 }}>
              <select style={{ ...S.select, width:"100%" }} value={form.responsavel} onChange={(e)=>upd("responsavel",e.target.value)}>
                {nomes.map((n)=><option key={n} value={n}>{n}</option>)}
              </select>
            </Field>
          </div>
          {form.responsavel!=="Igor" && (
            <Field label="Grau da correção">
              <DynSelect value={form.grau_correcao} onChange={(v)=>upd("grau_correcao",v)} options={opts.grau_correcao} placeholder="Selecione o grau..." onAdd={(v)=>{ setOpts((p)=>({ ...p, grau_correcao:[...p.grau_correcao,v] })); addOpt("grau_correcao",v); }}/>
            </Field>
          )}
          {form.responsavel!=="Igor" && (
            <div style={{ display:"flex", gap:10, alignItems:"flex-end" }}>
              <Field label="Feedback" style={{ flex:1 }}>
                <select style={{ ...S.select, width:"100%" }} value={form.feedback_tipo} onChange={(e)=>upd("feedback_tipo",e.target.value)}>
                  <option value="">Nenhum</option><option value="elogio">Elogiar</option><option value="critica">Criticar</option>
                </select>
              </Field>
              {form.feedback_tipo && (
                <Field label="Dado?" style={{ flex:0 }}>
                  <label style={{ display:"flex", alignItems:"center", gap:5, fontSize:12.5, cursor:"pointer", whiteSpace:"nowrap", padding:"7px 0" }}>
                    <input type="checkbox" checked={form.feedback_dado} onChange={(e)=>upd("feedback_dado",e.target.checked)}/> Feito
                  </label>
                </Field>
              )}
            </div>
          ...
          <div style={{ display:"flex", justifyContent:"flex-end", gap:8 }}>
            <button style={S.btn("ghost")} onClick={closeForm}>Cancelar</button>
...
// ─── ADMIN: DASHBOARD ─────────────────────────────────────────────────────────
function Dash({ registros, entregas, estagiarias }) {
  ...
  const stats = [
    { l:"Total Registros",  v:fil.length,                                   c:"#2563eb" },
    { l:"Trabalho Próprio", v:fil.filter((r)=>r.responsavel==="Igor").length, c:"#10b981" },
    { l:"Correções",        v:estR.length,                                   c:"#f59e0b" },
    { l:"Pendentes",        v:entregas.filter((e)=>e.status==="pendente").length, c:"#ef4444" },
  ];
    ...
    <div>
      ...
      <div style={{ display:"flex", gap:10, flexWrap:"wrap", marginBottom:18 }}>
        {stats.map((s)=>(
          <div key={s.l} style={S.statCard(s.c)}>
            <div style={{ fontSize:10.5, color:"#64748b", fontWeight:600, textTransform:"uppercase" }}>{s.l}</div>
      ...
      <div style={{ display:"flex", gap:14, flexWrap:"wrap", marginBottom:14 }}>
        <div style={{ ...S.card, flex:1, minWidth:300 }}>
          <h3 style={{ fontSize:13, fontWeight:700, marginBottom:8 }}>Por Responsável</h3>
          <ResponsiveContainer width="100%" height={200}><BarChart data={byResp}><XAxis dataKey="name" fontSize={11}/><YAxis fontSize={11}/><Tooltip/><Bar dataKey="value" radius={[4,4,0,0]}>{byResp.map((_,i)=><Cell key={i} fill={COLORS[i%COLORS.length]}/>)}</Bar></BarChart></ResponsiveContainer>
        </div>
        <div style={{ ...S.card, flex:1, minWidth:260 }}>
          <h3 style={{ fontSize:13, fontWeight:700, marginBottom:8 }}>Por Tipo de Manifestação</h3>
          <ResponsiveContainer width="100%" height={200}><PieChart><Pie data={byManif.slice(0,8)} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={70} fontSize={10} label={({name,value})=>`${name} (${value})`}>{byManif.slice(0,8).map((_,i)=><Cell key={i} fill={COLORS[i%COLORS.length]}/>)}</Pie><Tooltip/></PieChart></ResponsiveContainer>
        </div>
      </div>
      <div style={{ display:"flex", gap:14, flexWrap:"wrap", marginBottom:14 }}>
        <div style={{ ...S.card, flex:1, minWidth:300 }}>
          <h3 style={{ fontSize:13, fontWeight:700, marginBottom:8 }}>Crimes Mais Frequentes</h3>
          <ResponsiveContainer width="100%" height={200}><BarChart data={byCrime} layout="vertical"><XAxis type="number" fontSize={11}/><YAxis type="category" dataKey="name" fontSize={10} width={120}/><Tooltip/><Bar dataKey="value" fill="#8b5cf6" radius={[0,4,4,0]}/></BarChart></ResponsiveContainer>
        </div>
        <div style={{ ...S.card, flex:1, minWidth:260 }}>
          <h3 style={{ fontSize:13, fontWeight:700, marginBottom:8 }}>Grau de Correção</h3>
          <ResponsiveContainer width="100%" height={200}><PieChart><Pie data={byCorr} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={70} fontSize={10} label={({name,value})=>`${name} (${value})`}>{byCorr.map((_,i)=><Cell key={i} fill={COLORS[i%COLORS.length]}/>)}</Pie><Tooltip/></PieChart></ResponsiveContainer>
        </div>
      </div>
      <div style={S.card}>
        <h3 style={{ fontSize:13, fontWeight:700, marginBottom:8 }}>Volume por Dia</h3>
        <ResponsiveContainer width="100%" height={180}><LineChart data={byDay}><XAxis dataKey="data" fontSize={10}/><YAxis fontSize={11}/><Tooltip/><Line type="monotone" dataKey="total" stroke="#2563eb" strokeWidth={2} dot={{ r:3 }}/></LineChart></ResponsiveContainer>
      </div>
    </div>
  );
}
...
function InternReg({ entregas, setEntregas, opts, setOpts, userId, api, token, demo }) {
  ...
  const submit = async () => {
    ...
    const payload = {
      ...
      estagiaria_id:       userId,
      data_entrega:        today,
      hora_entrega:        now.toLocaleTimeString("pt-BR",{ hour:"2-digit", minute:"2-digit" }),
      status:              "pendente",
    };
    if (!demo) {
      try {
        ...
        setEntregas((prev)=>[...prev, { ...payload, id:Date.now() }]);
      }
    } else {
      setEntregas((prev)=>[...prev, { ...payload, id:Date.now() }]);
    }
  ...
  return (
    <div>
      ...
      {ok && (
        <div style={{ ...S.card, borderLeft:"4px solid #10b981", background:"#f0fdf4", display:"flex", alignItems:"center", gap:8 }}>
          <Check size={16} color="#10b981"/><span style={{ fontWeight:600, color:"#065f46", fontSize:13 }}>Entrega registrada com sucesso!</span>
        </div>
      )}
      <div style={{ ...S.card, maxWidth:480 }}>
        <Field label="Número do procedimento *"><input style={S.input} value={form.numero_procedimento} onChange={(e)=>upd("numero_procedimento",e.target.value)} placeholder="0000000-00.0000.0.00.0000"/></Field>
        <Field label="Tipo do procedimento *">
          <DynSelect value={form.tipo_procedimento} onChange={(v)=>upd("tipo_procedimento",v)} options={opts.tipo_procedimento} onAdd={(v)=>setOpts((p)=>({ ...p, tipo_procedimento:[...p.tipo_procedimento,v] }))}/>
        </Field>
        ...
        <div style={{ display:"flex", alignItems:"center", gap:16, marginBottom:10 }}>
          <label style={{ display:"flex", alignItems:"center", gap:5, fontSize:12.5, cursor:"pointer", color:"#ef4444", fontWeight:form.urgente?700:400 }}>
            <input type="checkbox" checked={form.urgente} onChange={(e)=>upd("urgente",e.target.checked)}/> Urgente
          </label>
          <span style={{ fontSize:12, color:"#94a3b8" }}>Registrado em: <strong>{new Date().toLocaleString("pt-BR")}</strong></span>
        </div>
        <button style={{ ...S.btn("primary"), width:"100%", justifyContent:"center", padding:"9px" }}
          onClick={submit} disabled={!form.numero_procedimento||!form.tipo_procedimento||!form.tipo_manifestacao}>
          <Send size={13}/> Registrar Entrega
        </button>
      </div>
    </div>
  );
    ...
    <div>
      ...
      <div style={S.card}>
        <table style={{ width:"100%", borderCollapse:"collapse" }}>
          <thead><tr>
            <th style={S.th}>Data</th><th style={S.th}>Hora</th><th style={S.th}>Procedimento</th>
            <th style={S.th}>Tipo</th><th style={S.th}>Manifestação</th><th style={S.th}>Status</th>
          </tr></thead>
          <tbody>
            {mine.map((e)=>(
              <tr key={e.id}>
...
function EstagiariaTab({ estagiarias, setEstagiarias, registros, entregas, onViewAs, api, token, demo }) {
  const emptyForm = { nome:"", email:"", tipo_estagiaria:"graduacao", carga_horaria_diaria:"4" };
  const [showForm, setShowForm] = useState(false);
  const [editId,   setEditId]   = useState(null);
  const [form,     setForm]     = useState(emptyForm);
  const upd = (k,v) => setForm((prev)=>({ ...prev, [k]:v }));
  ...
  const saveEst = async () => {
    ...
    if (editId) {
      ...
      setEstagiarias((prev)=>prev.map((e)=>e.id===editId?{ ...e,...patch }:e));
    } else {
      setEstagiarias((prev)=>[...prev, { id:"e"+Date.now(), ...patch, papel:"estagiaria", ativo:true }]);
    }
    closeForm();
  };
  ...
  const toggleAtivo = async (id) => {
    ...
    setEstagiarias((prev)=>prev.map((e)=>e.id===id?{ ...e, ativo:!e.ativo }:e));
  };
  ...
  return (
    <div>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:14 }}>
        <div>
          <h1 style={S.h1}>Estagiárias</h1>
          <p style={{ color:"#64748b", fontSize:13 }}>{estagiarias.filter((e)=>e.ativo).length} ativas de {estagiarias.length}</p>
        </div>
        <button style={S.btn("primary")} onClick={()=>{ setForm(emptyForm); setEditId(null); setShowForm(true); }}><Plus size={14}/> Nova Estagiária</button>
      </div>
      <div style={{ display:"flex", gap:14, flexWrap:"wrap" }}>
        {estagiarias.map((e)=>{
          ...
          return (
            <div key={e.id} style={{ ...S.card, flex:"1 1 280px", maxWidth:360, opacity:e.ativo?1:0.5 }}>
              ...
              <div style={{ display:"flex", gap:10, marginBottom:10 }}>
                {[
                  { v:regs.length, label:"Correções", c:"#2563eb" },
                  { v:graded.length>0?Math.round(nada/graded.length*100)+"%":"—", label:"Sem correção", c:"#10b981" },
                  { v:ents.filter((x)=>x.status==="pendente").length, label:"Pendentes", c:"#f59e0b" },
                ].map((stat)=>(
                  <div key={stat.label} style={{ flex:1, background:"#f8fafc", borderRadius:8, padding:"8px 10px", textAlign:"center" }}>
                    <div style={{ fontSize:18, fontWeight:700, color:stat.c }}>{stat.v}</div>
                    <div style={{ fontSize:10, color:"#64748b", textTransform:"uppercase" }}>{stat.label}</div>
                  </div>
                ))}
              </div>
              <div style={{ display:"flex", gap:6, justifyContent:"flex-end" }}>
                <button style={{ ...S.btn("ghost"), fontSize:11, padding:"5px 8px" }} onClick={()=>onViewAs(e.id)}><Eye size={12}/> Visualizar como</button>
                <button style={{ ...S.btn("ghost"), fontSize:11, padding:"5px 8px" }} onClick={()=>{ setEditId(e.id); setForm({ nome:e.nome, email:e.email||"", tipo_estagiaria:e.tipo_estagiaria, carga_horaria_diaria:String(e.carga_horaria_diaria) }); setShowForm(true); }}><Edit3 size={12}/> Editar</button>
      ...
      {showForm && (
        <Modal title={editId?"Editar Estagiária":"Nova Estagiária"} onClose={closeForm}>
          ...
          <div style={{ display:"flex", gap:10 }}>
            <Field label="Tipo" style={{ flex:1 }}>
              <select style={{ ...S.select, width:"100%" }} value={form.tipo_estagiaria} onChange={(e)=>upd("tipo_estagiaria",e.target.value)}>
                {TIPOS_EST.map((t)=><option key={t.value} value={t.value}>{t.label}</option>)}
              </select>
            </Field>
            <Field label="Carga horária/dia" style={{ flex:1 }}>
              <input style={S.input} type="number" step="0.5" min="1" max="8" value={form.carga_horaria_diaria} onChange={(e)=>upd("carga_horaria_diaria",e.target.value)}/>
            </Field>
          </div>
...
export default function App() {
  ...
  const loadData = useCallback(async (apiRef, tok, papel) => {
    ...
    try {
      const [regs, ents, profs, optsData, bl] = await Promise.all([
        ...
        apiRef.get("entregas",  tok, "order=data_entrega.desc,hora_entrega.desc"),
  ...
  // ── Restaura sessão após F5 (sessionStorage expira ao fechar a aba) ───────
  useEffect(() => {
    if (!ENV_API) return;
  ...
  const onAuth = async (tok, prof) => {
    sessionStorage.setItem("app_token",   tok);
    sessionStorage.setItem("app_profile", JSON.stringify(prof));
    setToken(tok); setProfile(prof);
    setActiveTab(prof.papel==="admin" ? "dia" : "registrar");
    setScreen("app");
    // Usa a api do argumento (pode ser ENV_API ou api do estado, ambos válidos aqui)
    await loadData(api || ENV_API, tok, prof.papel);
  };

  const logout = () => {
    sessionStorage.removeItem("app_token");
    sessionStorage.removeItem("app_profile");
    setDemo(false); setProfile(null); setToken("");
    setRegistros([]); setEntregas([]); setBacklog([]);
    setEstagiarias([]); setViewAs(null);
    setScreen(demo ? "config" : "auth");
  };
  ...
  if (screen==="auth")   return <AuthScreen api={api||ENV_API} onAuth={onAuth}/>;
  if (loading) return (
  ...
  return (
    <div style={S.page}>
      ...
      <div style={{ flex:1, display:"flex", flexDirection:"column" }}>
        ...
        {viewAs && <div style={{ ...S.demoBanner, background:"#dbeafe", color:"#1e40af" }}>
