...
import { LogOut, Plus, Check, Users, Send, History, X, Bell, Save, Trash2, ChevronLeft, ChevronRight, LayoutDashboard, ClipboardList, Edit3, Eye } from "lucide-react";
...
// ─── AUTH SCREEN — usa SDK diretamente ───
function AuthScreen({ sb, onAuth, sessionMsg }) {
  ...
  const submit = async () => {
    ...
    try {
      if (mode === "login") {
        ...
        const { data: profs } = await sb.from("profiles").select("*").eq("id", data.user.id);
        const prof = profs?.[0];
        ...
        if (!uid) { setErr("Erro ao criar conta. Tente fazer login."); setLoading(false); return; }
        await sb.from("profiles").insert({ id: uid, nome: f.nome, email: f.email, papel: "estagiaria", tipo_estagiaria: f.tipo, carga_horaria_diaria: parseFloat(f.carga) });
        const { data: profs } = await sb.from("profiles").select("*").eq("id", uid);
        const prof = profs?.[0];
  ...
  return (<div style={{ ...S.page, alignItems: "center", justifyContent: "center", background: "#0f172a" }}>
    ...
    <div style={{ background: "#fff", borderRadius: 16, padding: 36, width: 400, maxWidth: "90vw" }}>
      ...
      {sessionMsg && <div style={{ background: "#fef3c7", color: "#92400e", borderRadius: 8, padding: "10px 12px", fontSize: 12.5, fontWeight: 600, marginBottom: 14 }}>{sessionMsg}</div>}
...
function DiaTrabalho({ registros, setRegistros, entregas, setEntregas, backlog, setBacklog, opts, setOpts, crimes, setCrimes, estagiarias, selectedDate, setSelectedDate, sb, demo }) {
  ...
  const addOpt = async (campo, valor) => {
    if (!demo) try { await sb.from("opcoes").insert({ campo, valor }); } catch (e) {}
  };
  ...
  const saveReg = async () => {
    ...
    if (editId) {
      if (!demo) try { await sb.from("registros").update(clean).eq("id", editId); } catch (e) {}
      ...
      if (!demo) {
        try {
          const { data } = await sb.from("registros").insert(payload).select();
          setRegistros([...registros, data[0]]);
          if (corrigindo) {
            await sb.from("entregas").update({ status: "corrigido" }).eq("id", corrigindo.id);
  ...
  const delReg = async (id) => {
    ...
    if (!demo) {
      try {
        ...
        if (r?.entrega_id) await sb.from("entregas").update({ status: "pendente" }).eq("id", r.entrega_id);
      } catch (e) {}
    }
...
function InternReg({ entregas, setEntregas, opts, setOpts, crimes, setCrimes, userId, sb, demo }) {
  ...
  const submit = async () => {
    ...
    if (!demo) {
      try {
        const { data } = await sb.from("entregas").insert(payload).select();
        setEntregas([...entregas, data[0]]);
      } catch (e) { setEntregas([...entregas, { ...payload, id: Date.now() }]); }
...
function EstagiariaTab({ estagiarias, setEstagiarias, registros, entregas, onViewAs, sb, demo }) {
  const [showForm, setShowForm] = useState(false);
  const [editId, setEditId] = useState(null);
  ...
  const saveEst = async () => {
    ...
    if (editId) {
      ...
      if (!demo) try { await sb.from("profiles").update(patch).eq("id", editId); } catch (e) {}
  ...
  const toggleAtivo = async (id) => {
    ...
    if (!demo) try { await sb.from("profiles").update({ ativo: !est.ativo }).eq("id", id); } catch (e) {}
...
export default function App() {
  const [screen, setScreen] = useState(ENV_URL && ENV_KEY ? "auth" : "config");
  const [sb, setSb] = useState(() => ENV_URL && ENV_KEY ? initSupabase(ENV_URL, ENV_KEY) : null);
  ...
  const clearAppState = () => {
    setProfile(null);
    setRegistros([]); setEntregas([]); setEstagiarias([]); setBacklog([]);
    setOpts(INIT_OPTS); setCrimes([]);
  ...
  useEffect(() => {
    ...
    const { data: { subscription } } = sb.auth.onAuthStateChange((event) => {
      if (event === "SIGNED_OUT") {
        clearAppState();
        setSessionMsg("Sua sessão expirou. Por favor, faça login novamente.");
        setScreen("auth");
      }
    });
  ...
  const loadData = async (sbRef, papel) => {
    setLoading(true);
    try {
      const [regsRes, entsRes, profsRes, optsRes, blRes] = await Promise.all([
        ...
        papel === "admin" ? sbRef.from("backlog").select("*").order("data", { ascending: false }) : Promise.resolve({ data: [] }),
      ]);
      setRegistros(regsRes.data || []);
      setEntregas(entsRes.data || []);
      setEstagiarias(profsRes.data || []);
      setBacklog(blRes.data || []);
  ...
  const startDemo = () => {
    setDemo(true); setProfile(MOCK.profile); setRegistros(MOCK.registros); setEntregas(MOCK.entregas);
    setEstagiarias(MOCK.estagiarias); setBacklog(MOCK.backlog); setCrimes(MOCK.crimes);
  ...
  if (screen === "auth") return <AuthScreen sb={sb} onAuth={onAuth} sessionMsg={sessionMsg} />;
